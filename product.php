...
if (!empty($account_name)) {
    ...
    $stmt = $conn->prepare("SELECT AccountName FROM productline WHERE AccountName = ?");
    if ($stmt) {
        $stmt->bind_param("s", $account_name);
        $stmt->execute();
        $result = $stmt->get_result();
...
// Fetch Israel L2 Manager
$israel_l2_manager = "Invalid account name.";
$stmt = $conn->prepare("SELECT `Israel L2 Manager` FROM manager_pline WHERE AccountName = ?");
if ($stmt) {
    $stmt->bind_param("s", $account_name);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $israel_l2_manager = $row['Israel L2 Manager'];
    } else {
        $israel_l2_manager = "No Manager";
    }
    $stmt->close();
}

// Fetch India L2 Manager
$india_l2_manager = "Not Available";
$stmt = $conn->prepare("SELECT `India L2 Manager` FROM manager_pline WHERE AccountName = ?");
if ($stmt) {
    $stmt->bind_param("s", $account_name);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $india_l2_manager = $row['India L2 Manager'];
    }
    $stmt->close();
}

// Fetch Mexico L2 Manager
$mexico_l2_manager = "Not Available";
$stmt = $conn->prepare("SELECT `Mexico L2 Manager` FROM manager_pline WHERE AccountName = ?");
if ($stmt) {
    $stmt->bind_param("s", $account_name);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $mexico_l2_manager = $row['Mexico L2 Manager'];
    }
    $stmt->close();
}
    ...
    $information_list = [];
    $last_update = "";
    $stmt = $conn->prepare("SELECT information, updated_at FROM importantinformation WHERE AccountName = ?");
    if ($stmt) {
        $stmt->bind_param("s", $account_name);
        $stmt->execute();
        $result = $stmt->get_result();
...
<!-- End Navbar -->
        ...
        <h3 class="fw-bold mb-3"><?php echo htmlspecialchars($display_name); ?></h3>
			...
			<style>
			.custom-color {
				background-color: #faa175 !important; /* Change to any color */
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-7 col-stats">
                    <div class="text">
                        <p class="card-title" style="font-size: 15px;">Israel L2 Manager</p>
                        <h2 class="card-title" style="font-size: 15px;"><?php echo htmlspecialchars($israel_l2_manager); ?></h2>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-stats card-success card-round">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-7 col-stats">
                        <div class="numbers">
                            <p class="card-title" style="font-size: 15px;">India L2 Manager</p>
                            <h2 class="card-title" style="font-size: 15px;"><?php echo htmlspecialchars($india_l2_manager); ?></h2>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card card-stats card-secondary card-round">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-7 col-stats">
                        <div class="numbers">
                            <p class="card-title" style="font-size: 15px;">Mexico L2 Manager</p>
                            <h2 class="card-title" style="font-size: 15px;"><?php echo htmlspecialchars($mexico_l2_manager); ?></h2>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="chatbot-container" id="chatbot">
    ...
    <div class="chatbot-footer">
        ...
        <button id="chatbot-clear" class="btn btn-danger">Clear</button>
    </div>
</div>
